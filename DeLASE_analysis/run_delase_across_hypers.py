...
def compute_delase(cfg, session, run_params):
    ...
    delase = DeLASE(lfp,
        ...
        rank=max(ranks), # Use max rank for initial SVD
    ...
    results = []
    for rank in ranks:
        # Reuse SVD but compute HAVOK and stability for each rank
        ...
        log.info(f"Computing metrics for rank {rank}")
...
def main(cfg):
    ...
    try:
        ...
        log.info(f"Process ID {os.getpid()} executing task {cfg.params.session}, {cfg.params.area}, {cfg.params.run_index}, n_delays {cfg.params.n_delays}, rank {cfg.params.rank}, with {env}")
    except RuntimeError as e:
        # print(e)
        log.info(f"Process ID {os.getpid()} executing task {cfg.params.session}, {cfg.params.area}, {cfg.params.run_index}, n_delays {cfg.params.n_delays}, rank {cfg.params.rank} locally")
    ...
    results = compute_delase(cfg, cfg.params.session, run_params)
